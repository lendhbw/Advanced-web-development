...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "The resource could not be created because some input values are invalid. Please check the form fields and try again.";
  }

  const lines = errors.map((e) => {
    const field = formatFieldName(e.field);
    const msg = e.msg || "Invalid value";
    return `• ${field}: ${msg}`;
  });

  return `The resource could not be created because some fields contain invalid values. Please correct the form fields and try again.\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg = buildGenericErrorMessage(response.status, body);
    ...
    const msgLines = [];
    msgLines.push(
      `Resource "${body?.data?.name ?? ""}" was created successfully.`,
    );
    msgLines.push("You can now use it for bookings.");

    const details = [];
    if (body?.data?.id) details.push(`ID: ${body.data.id}`);
    if (createdAt) details.push(`Created at: ${createdAt}`);
    if (details.length > 0) {
      msgLines.push("");
      msgLines.push(details.join("\n"));
    }
