...
function RegisterForm() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!response.ok) {
        setSuccessMessage(
          data.error || "Something went wrong while sending data ❌",
        );
        setMessageType("error");
        return;
      }
    ...
    <div className="rounded-3xl bg-white p-8 shadow-soft">
      ...
      {messageType === "success" && (
        <div className="mt-6 w-full text-center">
          <Link
            to="/users"
            className="inline-block rounded-2xl bg-brand-blue px-5 py-3 text-sm font-semibold text-white shadow-soft transition-all duration-200 ease-out hover:bg-brand-dark/80"
          >
